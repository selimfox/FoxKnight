...
- 当任务涉及用户会反复编排或调试的场景、配置、参数、预制块或其他内容时，先识别用户实际采用的编辑方式，并把这些正常操作视为实现必须兼容的输入。
- 不得用只适配当前场景快照的坐标、节点名称、固定数量或隐藏参数修复工作流问题；需要先判断数据模型和运行时假设是否与用户的内容生产方式匹配。
- 宣布修复完成前必须追问并验证：“如果用户现在换一种正常编辑方式重新摆一次，这个修复还成立吗？”
- 验收除当前内容外，至少覆盖一种与本次问题相关的等价重编排，例如移动单个对象、预制块或父节点，增删、替换或重新组合内容。验证应证明用户无需同步维护未明确属于其职责的隐藏参数。
- 这组要求只在任务涉及用户持续编辑的内容时适用；固定内容、一次性资产或不提供作者编辑入口的其他工作不因此增加无关测试。
