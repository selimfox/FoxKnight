...
Player 负责移动、记录面向、接收鼠标按键意图，并显示由关卡控制器配置的临时瞄准预览。`AimPreview` 复用 Fill、CenterLine 与 Outline：直线斩绘制矩形；弧形斩绘制以 Player 当前原点为圆心的攻击圆，并用新增 `DeadzoneOutline` 绘制方向死区圆。死区外的 CenterLine 改绘从圆心沿鼠标方向延伸、包含箭头头部的位移指示，长度使用边界裁剪后的实际短位移；死区内隐藏箭头。Player 另外提供直线斩姿态与弧形旋斩姿态两个局部临时表现入口。攻击次数、刀型选择、边界裁剪与流程状态由 `PrototypeLevel` 统一裁决，避免多个脚本分别维护“一刀是否已用”。
...
唯一攻击由 `attack_committed: bool` 与状态共同保证：只有 `OBSERVING` 且尚未提交时可以进入 `AIMING`；进入瞄准不消耗攻击。`AIMING` 中每帧用狐狸到鼠标的距离与 `slash_form_distance_threshold` 比较，距离小于阈值选择 `ARC`，大于等于阈值选择 `STRAIGHT`。弧形斩继续比较鼠标距离与 `arc_direction_deadzone_radius`：小于死区半径时方向为零、位移为零；大于等于死区半径时使用狐狸到鼠标的归一化方向，并计算边界裁剪后的固定短位移。左键释放固定当前刀型、方向和距离，随后设置 `attack_committed`、切换到 `EXECUTING`、隐藏预览和锁定输入，再实例化斩击。右键取消优先；取消后随后到达的左键释放事件在 `OBSERVING` 中必须被忽略。
...
| 范围预览 | `Player/AimPreview` 按当前刀型显示直线矩形或弧形攻击圆；弧形额外显示可调死区圆，并在死区外显示鼠标方向与实际边界裁剪距离箭头。弧形预览不提前合并短位移路径，执行时由跟随刀光和逐帧圆形查询表达即时范围 | **v0.1临时实现，不代表最终设计** |
...
10. 在不改变状态机和唯一攻击门控的前提下接入 v0.1 刀型实验：先完成距离阈值模式选择与矩形/圆形双预览，再实现弧形逐帧圆形查询、跟随刀光、专属旋斩动作和提交瞬间方向采样，最后回归直线斩原逻辑、边界裁剪与 AC-01～AC-03、AC-06、AC-08～AC-10。
...
| P-08 镜头与可读性 | `FixedCamera`、`Player/AimPreview`、Prototype 场景构图 | 固定俯视镜头；预览按当前刀型使用实际扫掠参数与边界裁剪距离，不通过跟随或运行时平移修补构图 |
| P-09 结果反馈 | `player_controller.gd`、`prototype_slash.gd`、`feedback_controller.gd`、AnimationPlayer、AudioStreamPlayer | Player 区分直线姿态与弧形旋斩姿态；PrototypeSlash 绘制跟随圆弧刀光；其余反馈只消费已确定的战斗事件，不写入命中或胜败真值 |
...
| --- | --- | --- | --- |
| AC-01 | 启动基础关卡；分别在 `OBSERVING` 与 `AIMING` 移动狐狸；将鼠标置于刀型阈值和死区阈值两侧，改变鼠标角度及 WASD，观察双预览和固定镜头构图 | 移动、状态、模式、死区与箭头可自动；最终可读性人工确认 | 当前刀型、攻击圆/死区圆半径、箭头显隐/方向/边界裁剪长度断言；WASD 不改变箭头；场景截图、人工检查结论 |
...
| AC-06 | 分别执行全命中、部分命中、无命中三种布置，检查挥刀、命中、死亡、结算反馈；对比直线与弧形角色动作并捕获弧形短位移执行帧 | 动作模式和刀光跟随状态可自动；最终表现人工确认 | 事件日志、Player 最后攻击姿态、弧形刀光中心与 Player 坐标差、三种结果截图或短录屏、反馈未改写判定的回归结果 |
