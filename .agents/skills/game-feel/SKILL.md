---
name: game-feel
description: "为《狐狸骑士》诊断和改善已有动作的反馈、可读性与冲击力，包括命中停顿、震屏、闪光、缓动、形变和音画分层。仅在用户显式调用 `$game-feel`，或用户同意 Coding/Architecture Agent 基于试玩或可观察证据提出的专项反馈检查时使用；不得自动触发，不得改变命中、状态流或胜败真值。"
---

# Game feel (juice)

## FoxKnight invocation and authority

- Read the root `AGENTS.md`, the relevant `docs/GDD.md` feedback requirements, and the current `docs/TDD.md` before proposing or changing feedback.
- Use this skill only after explicit invocation, or after the user accepts an agent's concrete recommendation to run a focused game-feel pass.
- Diagnose the observed problem before proposing effects: weak impact, unclear state change, obscured survivor, poor timing, discomfort, or mismatch between visuals and truth.
- Treat GDD/TDD and gameplay truth as authoritative. Feedback consumes confirmed events; it never decides hits, deaths, round state, or results.
- Start with 2–3 feedback channels and compare them against the unchanged baseline. Do not mechanically apply every recipe or the 5–8-layer example.
- Keep every added effect local, adjustable, replaceable, and removable. Do not build a general feedback framework, settings system, or unrelated polish without confirmed scope.
- Separate observed evidence from predicted player feeling. Report what was actually run or viewed, and label unplayed recommendations as hypotheses.
